...
<select class="custom-select form-control" name="category_id" required>
  ...
  <option value="{{ $categories->id }}">{{$categories->name }} <span class="badge badge-primary"  data-placement="left" title="No. of post"> {{$categories->posts->count()}} </span> </option> @endforeach
</select>
</div>
