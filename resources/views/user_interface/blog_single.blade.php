...
<section class=" ">
  ...
  <a href="{{route('blog.author',$post->user->name)}}" type="button" class="btn btn-success btn-sm">
  <img src=" {{ asset('images/author.svg') }} "  width="20" height="20" alt="Author"> 
   <span class="badge badge-success">By {{ str_limit($post->user->name, 6) }} </span>
...
<button type="button" class="btn btn-success btn-sm">
  <img src=" {{ asset('images/tags.svg') }} "  width="20" height="20" alt="Tag"> <span class="badge badge-success">{{$post->tags->count()}}</span>
  <span class="sr-only">Tags</span>
</button>
    @foreach($post->tags as $tag)
    <a href="{{route('blog.tag',$tag->slug)}}" class="card-link btn btn-success btn-sm"> {{$tag->name}} </a>
    @endforeach
  </div>
...
<form method="POST" action="{{route('comment.store',$post->id)}}" autocomplete>
 ...
 <div class="m-2">
    ...
    <button type="submit" class="btn btn-success btn-sm card-link ">Submit</button>
  </div>
</form>
@endauth
...
<h2 class="lead btn btn-success btn-sm"> Most read post</h2>
</div>
<div class="row">
...
<div class="card ">
  ...
  <div class="card-body">
    ...
    <a href="{{route('blog.single',$most_read->slug)}}" class="btn btn-success btn-sm"> Read more </a>
    </div>
    <div class="card-footer">
     <button type="button" class="btn btn-success btn-sm">
    <img src=" {{ asset('images/chat.svg') }} "  width="20" height="20" alt="Comment">  <span class="badge badge-success"> {{$most_read->comments->count()}} </span>
  <span class="sr-only">Comment</span>
</button>

 <button type="button" class="btn btn-success btn-sm">
    <img src=" {{ asset('images/view.svg') }} "  width="20" height="20" alt="Views">  <span class="badge badge-success"> {{$most_read->view_count}} </span>
  <span class="sr-only">Views</span>
</button>
  </div>
  </div>
